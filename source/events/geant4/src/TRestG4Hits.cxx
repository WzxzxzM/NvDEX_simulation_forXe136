...
void TRestG4Hits::AddG4Hit(TVector3 pos, Double_t en, Int_t process, Int_t volume, Double_t pre_en, Double_t pos_en)
{
    XQAddHit(pos, en, 0, -1, -1, pre_en, pos_en);

    fProcessID.Set(fNHits);
    fVolumeID.Set(fNHits);
    fTime.Set(fNHits); // <<<< 新增一行：时间数组

    fProcessID[fNHits - 1] = process;
    fVolumeID[fNHits - 1] = volume;
    fTime[fNHits - 1] = 0; // <<<< 默认值设为 0，避免越界访问
}
...
void TRestG4Hits::AddG4Hit(TVector3 pos, Double_t en, Int_t process, Int_t volume,
                           Double_t pre_en, Double_t pos_en, Double_t time)
{
    XQAddHit(pos, en, 0, -1, -1, pre_en, pos_en);
    ...
    // std::cout << "AddG4Hit: Stored time = " << fTime[fNHits - 1] << " ns" << std::endl;
}
