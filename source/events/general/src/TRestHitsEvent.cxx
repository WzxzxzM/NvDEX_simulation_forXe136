...
void TRestHitsEvent::AddHit(Double_t x, Double_t y, Double_t z, Double_t en, Int_t particle_type, Double_t t, Short_t mod,
                            Short_t ch) {
    fHits->AddHit(x, y, z, en, particle_type, t, mod, ch);
}
...
TRestHits* TRestHitsEvent::GetXZHits() {
    ...
    for (int i = 0; i < this->GetNumberOfHits(); i++)
        if (IsNaN(this->GetY(i)))
            fXZHits->AddHit(this->GetX(i), this->GetY(i), this->GetZ(i), this->GetEnergy(i), 0, 0,
...
TRestHits* TRestHitsEvent::GetYZHits() {
    ...
    for (int i = 0; i < this->GetNumberOfHits(); i++)
        if (IsNaN(this->GetX(i)))
            fYZHits->AddHit(this->GetX(i), this->GetY(i), this->GetZ(i), this->GetEnergy(i), 0, 0,
                            this->GetModule(i), this->GetChannel(i));
...
TRestHits* TRestHitsEvent::GetXYZHits() {
    ...
    for (int i = 0; i < this->GetNumberOfHits(); i++)
        if (!IsNaN(this->GetX(i)) && !IsNaN(this->GetY(i)) && !IsNaN(this->GetZ(i)))
            fXYZHits->AddHit(this->GetX(i), this->GetY(i), this->GetZ(i), this->GetEnergy(i), 0, 0,
                             this->GetModule(i), this->GetChannel(i));
...
TPad* TRestHitsEvent::DrawEvent(TString option) {
    ...
    for (unsigned int n = 0; n < optList.size(); n++) {
        ...
        Double_t pitch = 1;  // Iris: for better visulization.
        if (endPos != string::npos) {
...
void TRestHitsEvent::DrawHistograms(Int_t& column, Double_t pitch, TString histOption) {
    ...
    histOption = "col";
    Int_t nBinsX = (fMaxX - fMinX) / pitch + 4;
    Int_t nBinsY = (fMaxY - fMinY) / pitch + 4;
    Int_t nBinsZ = (fMaxZ - fMinZ) / pitch + 4;

    fXYHisto = new TH2F("XY", "", nBinsX, fMinX - 2.5 * pitch, fMinX + pitch * nBinsX - 2.5 * pitch, nBinsY,
                        fMinY - 2.5 * pitch, fMinY + pitch * nBinsY - 2.5 * pitch);
    fXZHisto = new TH2F("XZ", "", nBinsX, fMinX - 2.5 * pitch, fMinX + pitch * nBinsX - 2.5 * pitch, nBinsZ,
                        fMinZ - 2.5 * pitch, fMinZ + (pitch)*nBinsZ);
    fYZHisto = new TH2F("YZ", "", nBinsY, fMinY - 2.5 * pitch, fMinY + pitch * nBinsY - 2.5 * pitch, nBinsZ,
                        fMinZ - 2.5 * pitch, fMinZ + (pitch)*nBinsZ);

    fXHisto = new TH1F("X", "", nBinsX, fMinX - 2.5 * pitch, fMinX + pitch * nBinsX - 2.5 * pitch);
    fYHisto = new TH1F("Y", "", nBinsY, fMinY - 2.5 * pitch, fMinY + pitch * nBinsY - 2.5 * pitch);
    fZHisto = new TH1F("Z", "", nBinsZ, fMinZ - 2.5 * pitch, fMinZ + pitch * nBinsZ - 2.5 * pitch);
