...
void TRestSignal::GetWhiteNoiseSignal(TRestSignal* noiseSgnl, Double_t noiseMean, Double_t noiseLevel) {
    this->Sort();

    for (int i = 0; i < GetNumberOfPoints(); i++) {
        TRandom3* fRandom = new TRandom3(0);

        noiseSgnl->AddPoint(GetTime(i), GetData(i) + fRandom->Gaus(noiseMean, noiseLevel));
