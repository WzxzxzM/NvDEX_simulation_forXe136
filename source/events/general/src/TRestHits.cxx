...
void TRestHits::XQAddHit( TVector3 pos, Double_t en, Double_t t, Short_t mod, Short_t ch, Double_t pre_en, Double_t pos_en)
{
    fNHits++;
    ...
    fPreEnergy.push_back( (Float_t) ( pre_en ) );
    fPosEnergy.push_back( (Float_t) ( pos_en ) );
    fEnergy.push_back((Float_t)(en));

    fTotEnergy += en;
}

void TRestHits::AddHit(Double_t x, Double_t y, Double_t z, Double_t en, Int_t particle_type, Double_t t, Short_t mod, Short_t ch) {
    fNHits++;
    ...
    fCh.push_back((Short_t)ch);
    fEnergy.push_back((Float_t)(en));
    fparticle_type.push_back((Int_t)(particle_type));
...
void TRestHits::AddHit(TRestHits& hits, Int_t n) {
    ...
    Int_t particle_type = hits.Getparticle_type(n);
    Double_t t = hits.GetTime(n);
    Short_t mod = hits.GetModule(n);
    Short_t ch = hits.GetChannel(n);

    AddHit(x, y, z, en, particle_type, t, mod, ch);
}
