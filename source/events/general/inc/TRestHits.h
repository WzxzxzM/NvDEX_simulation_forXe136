...
class TRestHits : public TObject {
   public:
    ...
    std::vector<Int_t> fparticle_type;  // [fNHits] particle_type at each
                                   // 0 is SeF6  1 is SeF5
    std::vector <Float_t> fPreEnergy; //->
    std::vector <Float_t> fPosEnergy; //->
	...
	void XQAddHit( TVector3 pos, Double_t en, Double_t t = 0, Short_t mod = -1, Short_t ch = -1, Double_t pre_en=0, Double_t pos_en=0);
    ...
    Int_t Getparticle_type(int n) { return ((Int_t)fparticle_type[n]); }  // return particle_type 0 is SeF5 , 1 is SeF6
    Double_t GetPreEnergy( int n ) { return ( (Double_t) fPreEnergy[n]); } //return value in keV
    Double_t GetPosEnergy( int n ) { return ( (Double_t) fPosEnergy[n]); } //return value in keV
