...
class TRestRawSignal : public TObject {
   ...
   protected:
    ...
    std::vector<Double_t> fSignalData;  // Vector with the data of the signal
   ...
   public:
    ...
    // Getters
    Double_t GetData(Int_t n) { return GetSignalData(n); }
    ...
    void AddPoint(Double_t d);
    void AddCharge(Short_t d);
    void AddDeposit(Short_t d);

    Double_t GetSignalData(Int_t n);
    ...
    void GetWhiteNoiseSignal(TRestRawSignal* noiseSgnl, Double_t noiseMean = 0., Double_t noiseLevel = 1.);
