...
Int_t TRestReadoutModule::FindChannel(Double_t absX, Double_t absY) {
    ...
    while (!this->isInsidePixel(channel, pixel, absX, absY)) {
        // cout<<"channel,pixel,absX,absY=("<<channel<<","<<pixel<<","<<absX<<","<<absY<<")"<<endl;
        count++;
        ...
        if (count > totalNodes) {
            // cout << "REST Error? I did not find any channel for hit position (" << absX << "," << absY << ")"
            //      << endl;

            for (int ch = 0; ch < GetNumberOfChannels(); ch++)
                for (int px = 0; px < GetChannel(ch)->GetNumberOfPixels(); px++)
                    if (isInsidePixel(ch, px, absX, absY)) {
                        cout << "( " << absX << " , " << absY << ") Should be in channel " << ch
                             << " pixel : " << px << endl;
...
TVector2 TRestReadoutModule::GetPixelCenter(TRestReadoutPixel* pix) {
    ...
    TVector2 corner2(GetPixelVertex(pix, 3));
...
void TRestReadoutModule::AddChannel(TRestReadoutChannel& rChannel) {
    for (int i = 0; i < rChannel.GetNumberOfPixels(); i++) {
        ...
        Double_t oX = rChannel.GetPixel(i)->GetVertex(2).X();
        Double_t oY = rChannel.GetPixel(i)->GetVertex(2).Y();
        Double_t sX = rChannel.GetPixel(i)->GetVertex(5).X();
        Double_t sY = rChannel.GetPixel(i)->GetVertex(5).Y();
