...
//______________________________________________________________________________
Double_t TRestElectronDiffusionProcess::SetBoundaries(TRestHitsEvent* evInput) {
    Double_t maxX = -1e10, minX = 1e10, maxZ = -1e10, minZ = 1e10, maxY = -1e10, minY = 1e10;

    TRestHits* fHits = evInput->GetHits();
    for (int nhit = 0; nhit < evInput->GetNumberOfHits(); nhit++) {
...
TRestEvent* TRestElectronDiffusionProcess::ProcessEvent(TRestEvent* evInput) {
    ...
    Double_t deltaZ = 0;  // mm
    // xq
    // Double_t centerZ = SetBoundaries(inputHitsEvent);
    // Double_t readoutZ = 1200;    // mm
    // Double_t targetZ = 500;      // mm
    // deltaZ = centerZ - targetZ;  // mm
    ...
    for (int n = 0; n < nHits; n++) {
        ...
        if (eDep > 0) {
            ...
            const Double_t z = hits->GetZ(n) - deltaZ;

            for (int p = 0; p < fReadout->GetNumberOfReadoutPlanes(); p++) {
                TRestReadoutPlane* plane = &(*fReadout)[p];
                // cout << plane->GetPosition().X() << " " << plane->GetPosition().Y() << " "
                // <<plane->GetPosition().Z() <<endl; cout << plane->GetCathodePosition().X() << " " <<
                // plane->GetCathodePosition().Y() << " " <<plane->GetCathodePosition().Z() <<endl;
...
void TRestElectronDiffusionProcess::InitFromConfigFile() {
    ...
    settingCoeff = StringToDouble(GetParameter("SettinglongitudinalDiffusionCoefficient", "0"));
