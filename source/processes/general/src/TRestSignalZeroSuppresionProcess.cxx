...
TRestEvent* TRestSignalZeroSuppresionProcess::ProcessEvent(TRestEvent* evInput) {
    ...
    if (fSignalEvent->GetNumberOfSignals() <= 0 && this->GetVerboseLevel() >= REST_Debug) {
      cout << "TRestSignalZeroSuppresionProcess: abnormal numbers of signals. (" << fSignalEvent->GetNumberOfSignals() << ")" << endl;
      return NULL;
    }

    //cout << " ---- " << fRawSignalEvent->GetIntegral() << " ! " << fSignalEvent->GetIntegral() <<endl;
    return fSignalEvent;
}
