...
TRestEvent *TRestSignalToRawSignalProcess::ProcessEvent(TRestEvent *evInput)
{
    ...
    if (GetVerboseLevel() >= REST_Debug && fInputSignalEvent->GetNumberOfSignals() <= 0)
    {
        cout << "TRestSignalToRawSignalProcess: abnormal numbers of signals. ("
             << fInputSignalEvent->GetNumberOfSignals() << ")" << endl;
        return NULL;
    }
    ...
    else if (fTriggerMode == "integralThreshold")
    {
        ...
        // cout << " ! " << minT << " ! " << maxT << " ! " << maxT - minT << " ! " <<endl;

        for (Double_t t = minT - fNPoints * fSampling; t <= maxT + fNPoints * fSampling; t = t + fSampling)
        {
            Double_t en = fInputSignalEvent->GetIntegralWithTime(t, t + (fSampling * fNPoints) / 2.);

            if (en > fIntegralThreshold)
            {
                // tStart = t - fTriggerDelay * fSampling < 0 ? minT - fTriggerDelay * fSampling : t -
                // fTriggerDelay * fSampling;
                tStart = TMath::Max(t - fTriggerDelay * fSampling, minT - fTriggerDelay * fSampling);
                tEnd = tStart + fNPoints * fSampling;
                // cout << t << " ! " << minT - fNPoints * fSampling << endl;
                // cout << minT << " ! " << fNPoints * fSampling << " ! " << t << " ! " << tStart << " ! "
                // << tEnd << endl;
                break;
            }
        }
    ...
    // Double_t totalValue = 0;
    // Double_t totalPoint = 0;
    // cout << tEnd - tStart << " ! " <<endl;
    // cout << " ! " << tStart << " ! " << tEnd << " ! " << tEnd - tStart << " ! " << endl;
    for (int n = 0; n < fInputSignalEvent->GetNumberOfSignals(); n++)
    {
        ...
        for (int m = 0; m < sgnl->GetNumberOfPoints(); m++)
        {
            ...
            if (t > tStart && t < tEnd)
            {
                ...
                // cout << timeBin << endl;
                // if (m < 20)
                // cout << timeBin << " ! " << t << " ! " << tStart << " ! " << tStart << " ! " << endl;
                if (GetVerboseLevel() >= REST_Warning)
        ...
        for (int x = 0; x < fNPoints; x++)
        {
            ...
            Double_t value = sData[x];
            // totalValue += value;
            rSgnl.AddPoint(value);
            // Double_t Point = rSgnl.GetData(x);
            // totalPoint += Point;
        }
        fOutputRawSignalEvent->AddSignal(rSgnl);
    }
    // cout << " ! " << fInputSignalEvent->GetIntegral() << " ! " << fOutputRawSignalEvent->GetIntegral()
    // <<endl;
