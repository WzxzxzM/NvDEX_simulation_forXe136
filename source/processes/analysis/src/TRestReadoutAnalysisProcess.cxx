...
TRestEvent* TRestReadoutAnalysisProcess::ProcessEvent(TRestEvent* evInput) {
    ...
    if (fReadout != NULL) {
        ...
        this->SetObservableValue("ReadoutChl_X", nan);
        this->SetObservableValue("ReadoutChl_Y", nan);
        ...
        if (firstX_id > -1 && firstY_id > -1) {
            ...
            this->SetObservableValue("ReadoutChl_X", channel1); // Form FirstX
            this->SetObservableValue("ReadoutChl_Y", channel2); // Form FirstY
        }
        double integral = 0;
        for (int i = 0; i < fSignalEvent->GetNumberOfSignals(); i++) {
