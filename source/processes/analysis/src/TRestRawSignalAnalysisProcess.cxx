...
TRestEvent* TRestRawSignalAnalysisProcess::ProcessEvent(TRestEvent* evInput) {
    ...
    vector<Int_t> trig_daqChls;
    vector<Int_t> trig_readoutChls;
    trig_daqChls.clear();
    trig_readoutChls.clear();
        ...
        if (sgnl->GetIntegralWithThreshold(from, to, fBaseLineRange.X(), fBaseLineRange.Y(), fPointThreshold,
            ...
            if (!fReadOnly && fReadout != NULL) {
                ...
                for (int s = 0; s < fSignalEvent->GetNumberOfSignals(); s++) {
                    ...
                    trig_daqChls.push_back(fSignalEvent->GetSignal(s)->GetID());
                    trig_readoutChls.push_back(readoutChannel);
                }
    ...
    SetObservableValue("Trig_DaqChls", trig_daqChls);
    SetObservableValue("Trig_ReadoutChls", trig_readoutChls);
