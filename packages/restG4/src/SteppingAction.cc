         ...
         volID++)
    {
      ...
      if (restG4Event->isVolumeStored(volID) || isDecay)
      {
        ...
        if (isActiveVolume || isDecay)
        {
          ...
          Double_t kinEnergyPreStep = prePoint->GetKineticEnergy();
          Double_t kinEnergyPostStep = postPoint->GetKineticEnergy();
          Double_t globalTime = prePoint->GetGlobalTime(); // 单位是 ns
          // G4cout << "Hit global time = " << globalTime << " ns" << G4endl;                                                 
          restTrack->AddG4Hit(hitPosition, ener_dep / keV, pcsID, volID, kinEnergyPreStep, kinEnergyPostStep, globalTime);
        }
      }
    }
  }
